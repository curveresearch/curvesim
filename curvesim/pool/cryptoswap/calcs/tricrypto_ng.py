...
def wad_exp(x: int) -> int:
    ...
    y: int = (
        (value + 1346386616545796478920950773328) * value
    ) >> 96 + 57155421227552351082224309758442
    p: int = (
        (((y + value) - 94201549194550492254356042504812) * y)
        >> 96 + 28719021644029726153956944680412240
    ) * value + (4385272521454847904659076985693276 << 96)

    # We leave `p` in the "2 ** 192" base so that we do not have to scale it up
    # again for the division.
    q: int = (
        (value - 2855989394907223263936484059900) * value
    ) >> 96 + 50020603652535783019961831881945
